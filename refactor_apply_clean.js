...
css = css.replace(/!\s+!/g, '!');
css = css.replace(/(dark|hover|focus|active|group-hover):\s+/g, '$1:');
...
css = css.replace(/^([ \t]*)@apply\s+(.+?);/gm, (match, indent, classesStr) => {
    ...
    for (const [variantCombo, utils] of Object.entries(groups)) {
        if (variantCombo === 'base' || utils.length === 0) continue;
    ...
    return newCss ? newCss.trimEnd() : '';
});

// Update the custom-variant which user broke
css = css.replace(/@custom-variant dark \(&:is\(\.dark \*\)\);/, '@variant dark (&:where(.dark, .dark *));');

fs.writeFileSync('src/app/globals.css', css, 'utf-8');
console.log('Refactored variants in globals.css successfully');
